...
public class SemanticAnalyzer {
	 ...
	 * 
	 * @param memLoc
	 *            string with format from VM definition e.g (0(D0), #"string",
	 *            etc)
	 */
	 ...
	 * @param dst
	 */
	private void subF(String src1, String src2, String dst) {
		out.println("subF " + src1 + " " + src2 + " " + dst);
	}
	...
	public void gen_activation_rec() {
		...
		add("SP", "#" + variableCount, "SP"); // reserveSpace for the variables
												// in the program
		register = "D" + (tbl.getNestingLevel());
		if (nestingLevel == 0) {
			sub("SP", "#" + (variableCount), register);
		} else {
			offset = "-" + (variableCount + parameterCount + 2) + "(SP)"; // one
																			// slot
																			// for
																			// return
																			// address,
																			...
																			// register
																			// value
			move(register, offset); // moves the current register into the space
									// reserved for the old register
			sub("SP", "#" + (variableCount + parameterCount + 2), register); // calculates
																				// the
																				// new
																				// register
																				// value
	...
	public void gen_deactivation_rec() {
		...
		if (nestingLevel == 0) {
			...
			move("0(SP)", register); // moves the old register value back into
										// the register
			add("SP", raOffset, "SP");
			ret();
		}
	...
	public void gen_id_increment(RecordType r, long offset, int nestingLevel) {
		if (r == RecordType.FLOAT) {
			addF(Long.toString(offset) + getNestingLevelString(nestingLevel),
					"#1.0", Long.toString(offset)
							+ getNestingLevelString(nestingLevel));
		} else if (r == RecordType.INTEGER) {
			add(Long.toString(offset) + getNestingLevelString(nestingLevel),
					"#1", Long.toString(offset)
							+ getNestingLevelString(nestingLevel));
		}
	}

	public void gen_id_decrement(RecordType r, long offset, int nestingLevel) {
		if (r == RecordType.FLOAT) {
			subF(Long.toString(offset) + getNestingLevelString(nestingLevel),
					"#1.0", Long.toString(offset)
							+ getNestingLevelString(nestingLevel));
		} else if (r == RecordType.INTEGER) {
			sub(Long.toString(offset) + getNestingLevelString(nestingLevel),
					"#1", Long.toString(offset)
							+ getNestingLevelString(nestingLevel));
		}
	}
